...
public class Panel_return_switcher_main_menu : MonoBehaviour {
	...
	void OnClick() //.. vid klick
	{
		...
		if(previousButton.GetComponent<Panel_switcher_main_menu>().Lerp_back == false && previousButton.GetComponent<Panel_switcher_main_menu>().Lerp == false) //Om förra panelen inte redan håller på med någon animation
		{
			previousButton.GetComponent<Panel_switcher_main_menu>().Lerp_back = true; //Skicka ett meddelande till förra panelen, Att starta tillbaka-animationen.
			
			previousButton.GetComponent<Panel_switcher_main_menu>().Return_pos = masterPanel.transform.localPosition + Button_move_distance; //Ge förra panelen positionen den ska gå tillbaka till.
